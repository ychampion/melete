...
| Reactions on a turn, either direction | closed on `lane/w16c-reactions` through `packages/contracts/src/reactions.ts`: the person taps one of four glyphs on an agent turn (`POST /messages/{seq}/reactions` with the turn's first event seq), the agent's glyph on the person's message is drawn under that bubble, and both are read from `GET /jobs/{conversation_id}/reactions` when a turn settles (a conversation is a job on the service as in the mock) | one addition so a glyph is live rather than read on settle: a `reaction { message_seq, emoji, by }` item on the experience stream, or the person's message seq on `conversationTurn` so a client can address it without inferring it from the turn's first event |
