"""Measure the thin Hermes configuration with the cell's execution toolset on.

Same method as measure_thin.py, same six stub tools, plus whatever is in the
file named by argv[3]. That file is written by `.agents/probe/execution_tools.ts`
straight from the connector manifests, so the schemas measured here are the ones
the broker serves rather than a copy that can drift from them.

Run it twice, once without argv[3] and once with, and the difference between the
two totals is what enabling execution costs in scaffolding.

    .hermes-venv/Scripts/python.exe .agents/probe/measure_exec.py         .hermes-src packages/skills/builtin/identity.md .agents/probe/execution_tools.json
"""
...
(HOME / "plugins" / "melete" / "__init__.py").write_text(textwrap.dedent('''
    ...
    import json, os
    extra = os.environ.get("MELETE_PROBE_EXTRA_TOOLS")
    if extra:
        TOOLS.extend([(name, description, schema) for name, description, schema in json.load(open(extra, encoding="utf-8"))])
