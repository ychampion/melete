/**
 * The refund journey, end to end: Melete finds money a company owes, the person
 * says "Handle it", one message goes out with their say-so, and the chase keeps
 * at it until the company pays.
 *
...
const key = 'refund-journey-integration-key-32-bytes';
...
async function refundFrom(domain: string) {
  ...
  if (!company || !item) throw new Error(`Expected a refund from ${domain}`);
...
async function handleIt(domain: string) {
  const { jobs } = fixture();
  const { company, item, text } = await refundFrom(domain);
...
withDb('a refund, from the scan that finds it to the reply that pays it', () => {
  beforeAll(async () => {
  ...
  test('1. the scan finds the refund, with a quote that holds, and the total counts it', async () => {
    ...
    const { map, item, text } = await refundFrom('thornfieldprint.example');
  ...
  test('2 and 3. one chase, one approved message, a stall, a follow-up, and the reply that pays', async () => {
    const { jobs } = fixture();
