 ...
 */
/**
 * The same resolution, against a transaction the caller already holds. The
 * broker calls this one: it is inside its own admission transaction when it
 * asks, and opening a second one here would have the two waiting on each other.
 */
export async function resolveTrustIn(
  tx: MemoryTx,
  scope: MemoryScope,
  input: { payload: Record<string, unknown>; handles: readonly string[] },
): Promise<TrustResolution> {
  const { payload, handles } = input;
  {
...
export function createTrustResolver(
  ...
  return {
    async resolve({ space_id, payload, handles }) {
      ...
      return sql.begin((tx) => resolveTrustIn(tx, scope, { payload, handles }));
    },
  };
}
