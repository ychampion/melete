...
The route middleware selects a space from `x-melete-space` or a query
parameter, with a single-space fallback, and then asks the service whether the
authenticated principal may use it (non-owners read only; records and skills
are filtered by audience). The test `asking for a different space than the
session holds is refused` supplies that header as its fixture context. This
file-view scope is separate from the memory routes' membership-derived scope.
...
against authoritative evidence` exercise those components. Every
Postgres-backed bootstrap mounts that router; a raw file edit alone does not
update Postgres claims.
