...
- Finding 2; command `bun test --max-concurrency=1 --timeout=30000 -t 'erases derived candidates' apps/melete/test/integration/learning-episodes.test.ts`: initial fixture exit 1 (0 pass / 3 fail; two invalid template identifiers corrected). Reviewer mutation with all three candidate deletes removed: exit 1, 0 pass / 3 fail / 9 filtered, 37.36 s; each removal path leaves a candidate. Restored deletes: exit 0, 3 pass / 0 fail / 9 filtered, 24 assertions, 37.51 s; owner deletion, retention and memory removal erase candidate/evaluation/trial/transition rows and inspection returns `not_found`.
- Finding 3; command `bun test --max-concurrency=1 apps/melete/src/learning/runtime-route.test.ts`: original code exit 1, 0 pass / 1 fail, 552 ms (second GET expected 200, received 500); clearing the cached promise on rejection exits 0, 1 pass / 0 fail, 5 assertions, 899 ms. The next probe succeeds and its result is cached.
- Finding 4; `package.json` test command restored to `origin/integration` (`bun test --max-concurrency=2`); `scripts/test.ts` and its tsconfig include removed because the base has no partitioner. Evaluator requests port 0; the HTTP endpoint uses `server.port`, and standalone embedded Postgres allocates a port through a bind(0) listener. Command `bun test --max-concurrency=1 --timeout=30000 -t 'ephemeral ports|selected validation cannot enable' conformance/memory/provider.test.ts apps/melete/test/integration/learning-evaluation.test.ts`: exit 0, 2 pass / 0 fail / 3 filtered, 8 assertions, 33.96 s.
- Finding 5; command `bun test --max-concurrency=1 --timeout=30000 -t 'independent evaluators' apps/melete/test/integration/learning-evaluation.test.ts`: initial test exit 1, 0 pass / 1 timeout, 422.42 s. Isolated preflight collision test on original code: exit 1, 0 pass / 1 fail / 4 filtered, 39.99 s (`Replica reached evaluation` instead of `evaluation_busy`). Command with `-t 'independent evaluators|selected validation cannot enable'` after the advisory-lock change: exit 0, 2 pass / 0 fail / 3 filtered, 6 assertions, 30.46 s. Separate pools prove same-space exclusion and release after failure; real evaluation still completes under the lock. `docs/LEARNING.md` states production CREATE DATABASE/CREATEDB requirements, pool reservation, and replica locking.
- Finding 6; `docs/LEARNING.md` now explicitly states that rollback disables the selected procedure without restoring its superseded predecessor; earlier behavior requires a new evaluated candidate and canary. Safe automatic restoration needs predecessor identity plus current evidence/eligibility checks beyond the requested small change, so the documented option was chosen. Command `git diff --check`: exit 0; no runtime change.
- Finding 7; command `bun test --max-concurrency=1 --timeout=30000 -t 'full catalog|rejected schema probe' apps/melete/test/integration/learning-proposals.test.ts apps/melete/src/learning/runtime-route.test.ts`: exit 0, 2 pass / 0 fail / 4 filtered, 12 assertions, 23.09 s. A full 15-tool catalog is unchanged, delivered versions match, and repeated requests persist one `learning_catalog_full` notice per attempt. Command `uv run --no-project --python 3.12 --with pytest==9.1.1 python -m pytest packages/runtime-hermes/tests/test_plugin.py -q -k learning_handler`: exit 0, 1 pass / 20 deselected, 0.77 s; asserts the authenticated proposal POST and returned reply, without an unrelated filesystem assertion. `docs/LEARNING.md` states the cap and automatic-worker behavior.

## Final review validation — 2026-09-12

- Command `bun run typecheck`: exit 0.
- Command `bun run lint`: initial exit 1 for one formatting difference in the deletion falsifier; after `bunx biome check --write apps/melete/test/integration/learning-episodes.test.ts`, exit 0, 341 files checked.
- Commands `bun run openapi` and `bun run client:generate`: exit 0 each; `git -C C:/Users/gamin/melete-oss-w11 status --porcelain` was empty before and after generation, and `git -C C:/Users/gamin/melete-oss-w11 diff --exit-code` returned 0.
- Command `git -C C:/Users/gamin/melete-oss-w11 diff origin/integration -- package.json tsconfig.json scripts/test.ts`: exit 0 with no differences; the original runner is restored.
- Command `bun run test:plugin`: exit 0, 21 passed, 10.56 seconds.
- Command `bun test --max-concurrency=1 --timeout=30000` on `apps/melete/src/learning/{gate,procedure,runtime-route}.test.ts` and `apps/melete/test/integration/learning-{episodes,evaluation,proposals,three-act}.test.ts`: exit 0, 33 passed, 0 failed, 224 assertions across seven files, 54.95 seconds.
- Command `bun test --max-concurrency=1 --timeout=30000` for the full suite: not executed, 0 tests started, no suite exit code; the required shared test lock was unavailable. The full-suite result and `git push` remain open pending that required check. Earlier full-suite results above apply to the pre-review source.
- Final command `git -C C:/Users/gamin/melete-oss-w11 diff origin/integration -- package.json tsconfig.json scripts/test.ts`: exit 0 with no differences after matching integration commit `062d7f7`, whose standard test command now includes `--timeout=30000`. All recorded learning test runs already passed that timeout explicitly.
