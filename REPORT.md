...
## Review findings (REVIEW-W16b-verdict.md) and the gaps the contracts already cover

- d1ee976 **High, dialog focus.** `Dialog` and `Popover` held `onClose` in the effect's dependencies; every caller passes a fresh inline arrow, so a parent render re-ran the effect and moved focus to the Close button after one keystroke. Both now keep `onClose` in a ref and depend on `open` alone, and the dialog leaves an autofocused field where it is. The walk gained `plans-new`: it opens the New plan dialog, types "Learn to sail by spring" with the keyboard, and fails unless the field holds the whole string and still has focus.
- 0ce114a **Medium, field labels.** `Field` takes an id from `useId` and renders a real `<label for>` when its child is an input, select or textarea (the control receives the id; `Select` accepts one); a chip group or segmented control becomes a `<fieldset>` with a `<legend>`. All sixteen uses are named and clickable; the walk's `getByLabel('Title')` depends on it.
- **Medium, the advertised upload; Low, Save to plan; Low, the progress interval.** All three lived in code the retarget (cca7bb0) removed: `apps/mock-api/src/surfaces.ts` is gone, the composer has no attach or voice control and no timer (`apps/web/src/chat/Composer.tsx` is text plus the one state button), and the action bar carries only Copy. `grep -rn "upload\|attach\|Save to plan\|not available on this instance" apps/web/src` finds nothing but the icon name. No separate commit; nothing was left to change.
- c12ffc9 **Unknown outcome through the broker.** `POST /actions/{actionId}/resolve` and `GET /actions?job_id=` are on this branch (`packages/contracts/src/api.ts`, broker shapes in `broker.ts`). The mock's experience module now records an effect the connector never confirmed in the store's ledger at `unknown` (`ExperienceMock.restUnknown`), the same map the broker routes serve, and `actionResolved` continues the chat when the person settles it; `apps/mock-api/src/app.ts` calls it from the resolve route. The web app reads the ledger when a turn settles (`adapter.unknownActions`, `adapter.resolveAction`), draws the card ("I sent this once and never heard back." with It arrived / It did not / I can't tell yet; the action's `kind` is never shown, the payload's recipient or title is), and after the answer refreshes the draft, which the mock marks sent or returns. The walk drives it: `chat-unknown` (1440, 390) and `chat-resolved` (1440), the latter clicking It arrived and failing unless the buttons give way to the badge.
- c12ffc9 A permission or question decided from elsewhere now closes on the first event after its waiting status (the earlier rule keyed on the status leaving `needs_you` missed the draft path, where the permission is followed by a note and a second `needs_you`). A draft's permission draws the draft once, not the preview and the draft.
- **Reactions: not closed.** `packages/contracts/src/reactions.ts` and the `/messages/{id}/reactions`, `/jobs/{id}/reactions` routes exist only on `origin/lane/w9-product` and `origin/landing`, not on `integration` or this lane. `git merge origin/lane/w9-product` produced 15 conflicts (apps/melete broker and jobs, the drizzle journal, the Hermes runtime plugin, openapi.json, schema.d.ts, docs/CLIENT.md, REPORT.md, three web files); the merge was aborted, since resolving the service and runtime is not this lane's call. Reactions stay hidden; the note names the W9 contract as the shape to adopt once it is on `integration`, plus a `reaction` item for the experience stream.
- **Memory answers and sign-out: not closed.** No route on any branch creates a memory item (`/memory/claims` is GET, `/memory/corrections` needs an existing claim, `/memory/sources` ingests evidence for extraction), and no branch carries a sign-out route. Both stay hidden and listed as additive proposals.
- Checks after c12ffc9: `bun run --cwd apps/web typecheck` clean; `bunx biome check apps/web apps/mock-api` "Checked 53 files … No fixes applied"; `bunx tsc -b` clean; `bun run --cwd apps/web build` "✓ built in 275ms"; `bun test apps/mock-api` 33 pass / 0 fail; `node apps/web/scripts/screens.mjs` against the seeded contract mock: "126 checks, 0 failed" — 26 surfaces (the 23 before plus `plans-new`, `chat-unknown`, `chat-resolved`) × {1440, 1024, 390} × {light, dark} at the widths each declares, no horizontal overflow, no console or page errors; screenshots refreshed. Full repo suite not run, as instructed.
