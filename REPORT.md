...
### 6. Keep listener ownership and match literal proposal prefixes

- Broker reproduction `bun test apps/melete/test/integration/runtime-handoff.test.ts --max-concurrency=1`: exit 1, 0 pass, 1 fail, 13 assertions. A same-job event with a lookalike prefix triggered `approval_hash_mismatch` through the unescaped underscore.
