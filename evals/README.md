...
The reply checks: a permission phrase counts as an unnecessary ask only when it asks leave for the task itself, meaning no statement precedes it or a required fact is still missing; after the answer it is reported as `reply does not close with an offer` instead. A forbidden value fails when the reply asserts it as current, not when it names it as replaced. A required phrase with a standalone number is met by that number. A naturalness word budget is never tighter than fifteen words, the identity's one short sentence, and a dash is not counted as a word.
