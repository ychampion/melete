...
Promotion requires better results and fewer required corrections on the target family, no regression on any tested template, full passes in critical families, and zero scope violations. A better family average cannot hide negative transfer on one template. Validation success alone cannot enable a procedure. Changing any bound part of a selected definition invalidates its evidence.
