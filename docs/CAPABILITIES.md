...
Production stdio launch rests on fixtures rather than on that proof: stdio
recovery and credential refresh are tested with explicit fixtures, and the
service refuses to launch an MCP child under its own OS identity, so a
production MCP server is an HTTP endpoint (see
[CONNECTORS](CONNECTORS.md#installed-mcp-servers)).
