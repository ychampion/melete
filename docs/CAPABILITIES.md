...
| MCP disconnect recovery | implemented-and-tested | The real Hermes proof terminates an HTTP session, then observes broker-managed reconnect, a fresh initialization, one call and a durable receipt. HTTP and stdio integration fixtures exercise the callback; repeated termination stops after three attempts. A lost acknowledgement stays unknown without replay. |
...
canary. Skipping the opt-in test is not end-to-end proof. The separate optional
