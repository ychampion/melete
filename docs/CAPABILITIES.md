...
| Remote sandbox on Daytona | implemented-and-tested against fixtures written from Daytona's published API | [daytona.test.ts](../apps/melete/src/sandbox/adapters/daytona.test.ts) replays the provider-neutral conformance suite (`a lost acknowledgement after the marker is unknown and is never re-run`, `deny-all egress holds from inside the sandbox`, `a symbolic link returned by the provider is refused` and the rest) and the workspace suite with `pause`, plus `a poll lost after the command started is unknown and the command is never run again`, `the key is sent only to the API and the configured toolbox proxy` and `a sandbox whose recorded egress differs from the request is refused and destroyed`. Replay fails on any missing, extra or different request. The live suite, [daytona.live.test.ts](../apps/melete/src/sandbox/adapters/daytona.live.test.ts), runs with `MELETE_SANDBOX_LIVE=daytona`; Daytona sets a sandbox's egress only for organisations on Tier 3 or Tier 4. This adapter has not yet been run against a lower tier. |
...
Docker engine by conformance 10, which CI runs on every pull request:
